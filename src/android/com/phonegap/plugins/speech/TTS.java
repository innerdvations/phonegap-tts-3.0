...
public class TTS extends CordovaPlugin implements OnInitListener, OnUtteranceCompletedListener {
    ...
    public boolean execute(String action, JSONArray args, CallbackContext callbackContext) {
        ...
        try {
            if (action.equals("speak")) {
            ...
            } else if (action.equals("synthesizeToFile")) {
                String text = args.getString(0);
                ...
                if (isReady()) {
                    ...
                    map = new HashMap<String, String>();
                    map.put(TextToSpeech.Engine.KEY_PARAM_UTTERANCE_ID, callbackContext.getCallbackId());
                    mTts.synthesizeToFile(text, map, file);
                    PluginResult pr = new PluginResult(PluginResult.Status.NO_RESULT);
                    pr.setKeepCallback(true);
                    callbackContext.sendPluginResult(pr);
                } else {
                ...
                result = Integer.toString(state);
                callbackContext.sendPluginResult(new PluginResult(status, result));
            }
            else if (action.equals("isSpeaking")) {
                if (mTts != null) {
                    result = mTts.isSpeaking() ? "true" : "false";
                    callbackContext.sendPluginResult(new PluginResult(status, result));
                }
            }
            else if (action.equals("getMaxSpeechInputLength")) {
                if (mTts != null) {
                    result = Integer.toString(mTts.getMaxSpeechInputLength());
                    callbackContext.sendPluginResult(new PluginResult(status, result));
                }
            }
            return true;
